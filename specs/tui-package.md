...
```text
packages/opencode ---\
                      > @self-opencode/tui -> @self-opencode/sdk
packages/cli --------/
```
...
`packages/cli`, or `@self-opencode/core`.
...
- Do not replace private `packages/opencode` imports with `@self-opencode/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@self-opencode/tui`.

## Ownership Boundary

### `@self-opencode/tui` Owns
...
Status: Completed. The private `@self-opencode/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@self-opencode/tui`.
...
- The package has no dependency on `opencode`, `@self-opencode/cli`, or
  `@self-opencode/core`.
...
primitives now live in `@self-opencode/tui`. The legacy host consumes explicit
...
and build inputs through `@self-opencode/tui/runtime`. Movable app, component,
...
`@self-opencode/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@self-opencode/tui`. Configured references resolve through the new
...
`@self-opencode/tui`. The legacy host injects a narrow plugin host that retains
...
public `@self-opencode/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@self-opencode/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@self-opencode/core`, or either
  executable package.
...
rg '@self-opencode/core|packages/opencode|packages/cli' packages/tui
