...
- Author package entrypoint is `@self-opencode/plugin/tui`.
...
- Import types from `@self-opencode/plugin/tui`.
- `@self-opencode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@self-opencode/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@self-opencode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@self-opencode/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@self-opencode/plugin/tui`.
